...
#include <cstring>
using namespace std;
void getAllFilesPath(string path, vector<string>& filesName);//获取完整的文件名
void getAllFilesName(string path, vector<string>& filesName);//只获取在目录下的所有文件名
void writeFilesName(const vector<string>& filesName);//用文件的读取方式
void getAllFilesPath(string path, vector<string>& filesName) {
    long hFile = 0; //文件句柄
    struct _finddata_t fileinfo;//文件信息
    string tempFilePath;//文件搜索指针
